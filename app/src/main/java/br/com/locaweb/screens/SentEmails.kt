...
@Composable
fun SentEmails(navController: NavController) {
    ...
    LaunchedEffect(Unit) {
        val call = RetrofitClient.getServiceWithToken(context).getSentEmails()
            ...
            LazyColumn {
                items(emailsState) { email ->
                    MailBox(
                        ...
                        subTitle = "Enviado para ${email.destination}",
